...
```{r, echo=FALSE}
plot.state(edu01a_parsed$state)
```

## plot.county

```{r}
plot.county <- function(data, column = "enrollment", state = "NC", direction = "top", n = 5){
  ...
  data_plotted = state_data |>
    ...
    geom_point() +
    labs(
      title = paste((direction), n, "counties in", state),
      x = "Year",
      y = column,
      color = "County"
    )
