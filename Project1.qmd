...
```{r}
parse_data_wrapper <- function(data, column = "enrollment"){
  result = select_pivot(data, column) |> #function 1 and 2
    extract_year_id() |> # function 3
    identify_locations() #function 4,5,6
  return(result)
}

## test the function
result = parse_data_wrapper(edu01a)
county_testa = result$county
state_testa = result$state
...
## Call and Combining the Data

```{r}
#calling the wrapper for A
edu01a_parsed = parse_data_wrapper("https://www4.stat.ncsu.edu/~online/datasets/EDU01a.csv")
#Calling the wrapper for B
edu01b_parsed = parse_data_wrapper("https://www4.stat.ncsu.edu/~online/datasets/EDU01b.csv")

#combine the results of the wrapper function
parse_data_wrapper_combined = function(resulta, resultb) {
  county_join = dplyr::bind_rows(resulta$county, resultb$county)
  state_join = dplyr::bind_rows(resulta$state, resultb$state)
  
  return(list(county = county_join, state = state_join))
}

#joining wrapper results from a and b
edu01a_b_join = parse_data_wrapper_combined(edu01a_parsed, edu01b_parsed)
...
```{r}
plot.state <- function(data, column = "enrollment"){
  data |> 
    filter()
}
